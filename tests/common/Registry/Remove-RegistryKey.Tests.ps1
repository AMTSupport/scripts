Describe "Remove-RegistryKey Tests" -Skip:(-not $IsWindows) {
    ...
    Context "Basic Functionality" {
        It "Should remove registry key when it exists" {
            ...
            Should -Invoke Test-RegistryKey -Exactly 1 -Scope It
            Should -Invoke Remove-ItemProperty -Exactly 1 -Scope It
        }
        ...
        It "Should not attempt to remove when key does not exist" {
            ...
            Should -Invoke Test-RegistryKey -Exactly 1 -Scope It
            Should -Invoke Remove-ItemProperty -Exactly 0 -Scope It
        }
    ...
    Context "Edge Cases" {
        It "Should handle special characters in key names" {
            ...
            Should -Invoke Remove-ItemProperty -ParameterFilter { $Name -eq 'Special-Key_Name.123' }
        }
    ...
    Context "Registry Hive Support" {
        It "Should work with HKLM registry hive" {
            ...
            Should -Invoke Test-RegistryKey -ParameterFilter { $Path -like 'HKLM:*' }
        }
        ...
        It "Should work with HKCU registry hive" {
            ...
            Should -Invoke Test-RegistryKey -ParameterFilter { $Path -like 'HKCU:*' }
        }
    }
}
