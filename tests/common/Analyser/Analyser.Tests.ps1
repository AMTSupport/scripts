...
Describe 'Analyser Module Tests' {
    ...
    Context 'Error Handling' {
        It 'Should handle null and empty CheckType' {
            $NullAttr = [Compiler.Analyser.SuppressAnalyserAttribute]::new($null, 'TestData')
            $NullAttr.CheckType | Should -Be ''
            $NullAttr.Data | Should -Be 'TestData'
            
            $EmptyAttr = [Compiler.Analyser.SuppressAnalyserAttribute]::new('', 'TestData')
            $EmptyAttr.CheckType | Should -Be ''
            $EmptyAttr.Data | Should -Be 'TestData'
        }
    }
}