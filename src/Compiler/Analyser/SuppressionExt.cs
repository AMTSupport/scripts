...
public static partial class SuppressAnalyserAttributeExt {
    [return: NotNull]
    public static Fin<IEnumerable<SuppressAnalyserAttribute>> FromAttributes(IEnumerable<AttributeAst> attributes) {
        ...
        foreach (var attr in attributes) {
            ...
            if (suppressionResult.IsErr(out var err, out var suppressionOpt)) {
                ...
                continue;
            } else if (suppressionOpt.IsSome(out var suppression)) suppressions.Add(suppression);
        }
    ...
    public static Fin<Option<SuppressAnalyserAttribute>> FromAttributeAst([NotNull] AttributeAst attrAst) {
        ...
        if (positionalArguments != null && positionalArguments.Count != 0) {
            ...
            switch (count) {
                case 3:
                    if (positionalArguments[2] is not StringConstantExpressionAst justificationAst) return IncorrectDataType("Justification", "string constant", positionalArguments[2].Extent);
                ...
                case 2:
                    ...
                    goto case 1;
                case 1:
                    if (positionalArguments[0] is not StringConstantExpressionAst checkTypeAst) return IncorrectDataType("CheckType", "string constant", positionalArguments[0].Extent);
        ...
        return Some(new SuppressAnalyserAttribute(checkType, data) {
